...
describe('StationSong Model', function () {
  ...
  it('finds active stationSongs', async function () {
    ...
    let foundStationSongIds = foundStationSongs.map((stationSong) => stationSong.id);
    let activeStationSongIds = stationSongs.slice(2).map((stationSong) => stationSong.id);
